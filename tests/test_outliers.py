...
def make_fake_data():
    ...
    test_df = pd.DataFrame(
        np.random.randint(100, size=(100, 3)), columns=["Group", "Group2", "count"]
    )
    test_df["group_letter"] = np.where(test_df["Group"] > 50, "A", "B")
    test_df["colour"] = pd.qcut(
        test_df["Group2"], 5, labels=list(["Red", "Blue", "Green", "Orange", "Yellow"])
    )
...
def test_get_stats():
    ...
    # assert statements
    assert (
        results_df.iloc[0]["mean"] == test_res[0]
    ), "mean does not match expected value"
    assert (
        round(results_df.iloc[0]["std"], 6) == test_res[1]
    ), "standard deviation does not match expected value"
